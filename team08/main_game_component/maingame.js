...
NEXT_BUTTON.addEventListener("click", () => {
  const RESULT_TEXT = document.querySelector(".result-text[style*='display: block;']");
  const SELECTED_BUTTON = document.querySelector(".option-btn[style*='var(--color-correct)'], .option-btn[style*='var(--color-incorrect)']");

  if (RESULT_TEXT) RESULT_TEXT.style.display = "none";
  if (NEXT_BUTTON) NEXT_BUTTON.style.display = "none";
  if (SELECTED_BUTTON) SELECTED_BUTTON.style.borderColor = "var(--color-primary)";
...
function buttonUpdateSelection(assigned) {
  ...
  const SELECTED_BUTTON = document.querySelector(`#button${parseInt(assigned)}`);
  const NEXT_BUTTON = document.querySelector("#next-btn")
